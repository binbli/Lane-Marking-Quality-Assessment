function
eval_sub(data_name, data_root, eval_list, model_weights, model_deploy, fea_cha, crop_size_h, crop_size_w, data_class,
         data_colormap, ...
...
load(data_colormap);
if (~
isdir(save_gray_folder)
)
mkdir(save_gray_folder);
end
if (~
isdir(save_color_folder)
)
mkdir(save_color_folder);
end
if (~
isdir(save_feat_folder)
&& is_save_feat)
...
score = score { 1 };
score = permute(score, [2 1
3]);
score = exp(score);
score = bsxfun(@rdivide, score, sum(score, 3));
data = imresize(score, [size(img, 1) size(img, 2)
], 'bilinear');
...
imwrite(imPred,
[
save_gray_folder img_fn
'.png']);
imwrite(imPred, colormapcs,
[
save_color_folder img_fn
'.png']);
end

if (is_save_feat)
save([
save_feat_folder img_fn
], 'data');
end
        end
caffe.
