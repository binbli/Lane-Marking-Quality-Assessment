...
namespace LD {
    ...
    void LMsfromCam::ParseXML() {
        ...
        if (m_dataRoot.empty() || m_dataFile.empty() || m_segRoot.empty() || m_refinedRoot.empty() ||
            (m_saveVizImg && m_vizImgPrefix.empty()) || m_refinedImgPrefix.empty())
            throw runtime_error(
                    "One of the following attributes are missing in SolverInstance node of Segmenter: dataRoot, dataFile, segRoot, refinedRoot, vizImgPrefix, saveVizImg, refinedImgPrefix");
    }
